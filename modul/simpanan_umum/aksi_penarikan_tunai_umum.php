...
// Hapus tipe
if ($module=='penarikan_tunai_umum' AND $act=='delete'){
  mysql_query("DELETE FROM tbl_trans_sp WHERE id='$id'");
  echo json_encode(array('ok' => true, 'msg' => '<div class="text-green"><i class="fa fa-check"></i> Data berhasil disimpan </div>'));
}
// Get Data Jenis Simpanan
elseif ($module=='penarikan_tunai_umum' AND $act=='getDataJenis'){
...
// Get Data Anggota
elseif ($module=='penarikan_tunai_umum' AND $act=='getDataAnggota'){
  $q=$_POST['q'];
  $kel=$_GET['kel'];  
 // var_dump($kel);exit;
  $sql="select fnId, ftNoRekening, ftNamaNasabah,ftAlamat,ftJabatan,ftNamaKelompok FROM tlnasabah WHERE fnTipeNasabah='UMUM' AND ftNamaNasabah like '%".$q."%' and ftKantorBayar='$kel'";
//  print_r($sql);
  $i	= 0;
  $rows   = array(); 
...
elseif ($module=='penarikan_tunai_umum' AND $act=='getDataAnggota_id'){
  $q=$_POST['q'];
...
elseif ($module=='penarikan_tunai_umum' AND $act=='getdata'){
 ...
 $sql="SELECT a.*,b.jns_simpan,c.nama,d.ftNamaNasabah,d.ftJabatan, d.ftNoRekening 
  ...
  $result2=mysql_query($sql);
	  while($r=mysql_fetch_array($result2)){
		...
		$rows[$i]['nota'] = '<p></p><p>
		<a href="'.'modul/simpanan_umum/aksi_penarikan_tunai_umum.php?module=penarikan_tunai_umum&act=cetak_penarikan&id_simpan='.$r[id].'" title="Cetak Bukti Transaksi" target="_blank"> <i class="glyphicon glyphicon-print"></i> Nota </a></p>';
...
elseif ($module=='penarikan_tunai_umum' AND $act=='create'){
...
elseif ($module=='penarikan_tunai_umum' AND $act=='update'){
...
}elseif ($module=='penarikan_tunai_umum' AND $act=='cetak'){
	    include "../../pdf/pdf.php";
...
}elseif ($module=='penarikan_tunai_umum' AND $act=='cetak_penarikan'){
