...
// Hapus angsuran-pensiun
if ($module=='angsuran_pensiun' AND $act=='delete'){
  mysql_query("DELETE FROM txangsuran_pensiun_hdr WHERE fnid='$_GET[fnid]'");
  header('location:angsuran-pensiun.html');
}

// Input angsuran-pensiun
elseif ($module=='angsuran_pensiun' AND $act=='input'){
 
	  mysql_query("INSERT INTO txangsuran_pensiun_hdr(		ftTrans_No,
	  														ftBranch_Code,
															fdTrans_date,
															fcSimpanan,
															ftNotes,
	   ...
	   mysql_query("UPDATE txpinjaman_pensiun_hdr set fcOutstanding = fcOutstanding - '$fcPokokAngsuran' where ftTrans_No='$_POST[ftLoan_No]' and ftCustomer_Code='$_POST[ftCustomer_Code]'");

  header('location:angsuran-pensiun.html'); 

}

// update angsuran-pensiun
elseif ($module=='angsuran_pensiun' AND $act=='update'){
 
	  mysql_query("UPDATE txangsuran_pensiun_hdr SET	fnStatus = '99',
