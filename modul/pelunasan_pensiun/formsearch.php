...
<html>
	<head>
		...
		<title>Tabel Transaksi Pensiun</title>
		...
		function pick(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p) {
		  if (window.opener && !window.opener.closed){
			...
			window.opener.document.myForm.fcPbltAngsuran.disabled=false;
		  }
		  window.close();
		}
		</script>
...
<table id="formsearch" class="table table-striped " onmouseover="this.style.cursor='pointer'">
	  ...
	  <thead>
		...
		<tbody>
				...
				$tampil=mysql_query("SELECT a.*, b.fcPokokAngsuran * a.sisa AS fcPokokAngsuran, 
						b.fcBunganAngsuran * a.sisa AS fcBunganAngsuran,  
						b.fcAdmAngsuran * a.sisa AS fcAdmAngsuran, 
						b.fcPbltAngsuran * a.sisa AS fcPbltAngsuran, 
						b.fcTabAngsuran * a.sisa AS fcTabAngsuran,(b.fcPokokAngsuran + b.fcBunganAngsuran + b.fcAdmAngsuran + b.fcPbltAngsuran + b.fcTabAngsuran) * a.sisa AS fcTotalAngsuran,
					b.fcplafond, b.ffbunga, fnJW, c.ftAlamat, c.ftNamaKelompok,c.ftKantorBayar,c.ftNamaNasabah,c.ftNoRekening, b.fdTrans_date
					FROM (
							SELECT xx.ftCustomer_Code, xx.ftTrans_No, SUM(xx.fcPlafond) AS fcOutstanding, SUM(xx.fcPlafond)/SUM(xx.fcPokokAngsuran) AS sisa
							FROM (
								SELECT ftCustomer_Code, ftTrans_No, fcPlafond , fcPokokAngsuran
								FROM txpinjaman_pensiun_hdr
								WHERE fnstatus = 1
						
								UNION ALL

								SELECT IFNULL(a.ftCustomer_Code,'') AS ftCustomer_Code, IFNULL(a.ftTrans_No,'') AS ftTrans_No,
								-IFNULL(b.fcPokokAngsuran,0), 0
								FROM txpinjaman_pensiun_hdr a
								LEFT JOIN txangsuran_pensiun_hdr b ON a.ftCustomer_Code = b.ftCustomer_Code AND a.ftTrans_No = b.ftloan_No
								AND b.fnStatus = 1
								WHERE a.fnstatus = 1
								...
								-IFNULL(b.fcPokokAngsuran,0), 0
								FROM txpinjaman_pensiun_hdr a
								LEFT JOIN txpelunasan_pensiun_hdr b ON a.ftCustomer_Code = b.ftCustomer_Code AND a.ftTrans_No = b.ftloan_No
								AND b.fnStatus = 1
								WHERE a.fnstatus = 1
					...
					LEFT JOIN txpinjaman_pensiun_hdr b ON a.ftCustomer_Code = b.ftCustomer_Code AND a.ftTrans_No = b.ftTrans_No
