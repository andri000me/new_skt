...
?>
 ...
 <?php
if($kelompok !="all"){
    $header=mysql_query("SELECT b.ftNamaKelompok AS ftkelompok,a.fdTrans_date FROM txpinjaman_mikro_nasabah_hdr a LEFT JOIN tlnasabah b ON a.ftCustomer_Code = b.ftNoRekening WHERE a.fnStatus <> 9 AND  b.ftNamaKelompok='$kelompok' AND a.fdTrans_date between '$startDate' AND '$endDate' GROUP BY ftkelompok ");
            $w=mysql_fetch_array($header);
            $e=$w[fdTrans_date];
  $temp2 ="<center><b>LAPORAN PENYALURAN KREDIT MIKRO<br>TANGGAL : $e<br>KELOMPOK :$kelompok";
...
//$tampil=mysql_query("SELECT * FROM txpinjaman_mikro_hdr WHERE fdTrans_date between '$startDate' AND '$endDate'");
    $tampil=mysql_query("SELECT a.ftCustomer_Code , a.fdTrans_date, b.ftNamaNasabah, b.ftSubCabang AS ftwilayah, b.ftNamaKelompok AS                ftkelompok,
                    a.fnJW, a.fcPlafond, a.fcPokokAngsuran + a.fcBunganAngsuran + a.fcAdmAngsuran + a.fcPbltAngsuran AS fcAngsuran,
                    a.fcTabAngsuran, a.fcTotalAngsuran, a.fcAdm, a.fcAsuransi,a.fcSimpanan, a.fcMaterai, a.fcPblt,
                    a.fcAdm + a.fcAsuransi + a.fcSimpanan+ a.fcMaterai+ a.fcPblt AS fcTotalPotongan,
                    a.fcTerimaBersih 
                    FROM txpinjaman_mikro_nasabah_hdr a
                    LEFT JOIN tlnasabah b ON a.ftCustomer_Code = b.ftNoRekening
                    WHERE a.fnStatus <> 9 AND b.ftNamaKelompok='$kelompok' AND a.fdTrans_date between '$startDate' AND '$endDate' 
                ");
    $no1 = 0;
    while($r=mysql_fetch_array($tampil)){
        ...
        $fcAsuransi=format_rupiah($r['fcAsuransi']);
 ...
 $temp2 .="   <tr>
        ...
        <td>$fcPblt</td>
        <td>$fcTabAngsuran</td>
        <td>$fcTotalPotongan</td>
        <td>$fcTerimaBersih</td>
   </tr>";
