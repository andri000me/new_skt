...
$fcTotalPelunasanTab =str_replace(',', '',$_POST['fcTotalPelunasanTab']);
$fcTotalPelunasan =str_replace(',', '',$_POST['fcTotalPelunasan']);
$ftTrans_No_old =str_replace(',', '',$_POST['ftTrans_No_old']);
...
// Hapus transaksi
if ($module=='transaksi_umum' AND $act=='delete'){
  $sql=mysql_fetch_array(mysql_query("select ftTrans_No,ftCustomer_Code from txpinjaman_umum_hdr where fnid ='$_GET[fnid]'"));
  $fttransno=$sql["ftTrans_No"]; 
  $ftCustomer_Code=$sql["ftCustomer_Code"];
  mysql_query("DELETE FROM txtagihan WHERE ftTrans_No='$fttransno' and ftCustomer_Code='$ftCustomer_Code'");
  mysql_query("DELETE FROM txpinjaman_umum_hdr WHERE fnid='$_GET[fnid]'");
  header('location:transaksi-umum.html');
}

// Input transaksi
elseif ($module=='transaksi_umum' AND $act=='input'){
	$sql=mysql_query("SELECT ftTrans_No FROM txpinjaman_umum_hdr WHERE ftTrans_No='$_POST[ftTrans_No]' ");
	...
	if($duplicate != 0){
		...
		$thn = date("Y");
		$thn2 = substr($thn, -2);
		$cariid=mysql_query("SELECT max(fnid) AS notrans FROM txpinjaman_umum_hdr");
		...
		$ftTrans_No= "PJU$thn2$bln$unik2";
	}else{
		$ftTrans_No= $_POST['ftTrans_No'];
	}
	  mysql_query("INSERT INTO txpinjaman_umum_hdr(
												ftTrans_No,
												ftBranch_Code,
												fdTrans_date,
												fcSaldosimpanan,
 ...
 if($_POST['ftTrans_No_old']!=''){
	...
	$w=	 mysql_query("INSERT INTO txangsuran_umum_hdr(ftLoan_No,
												ftBranch_Code,
												fcPlafond,
												fcPokokAngsuran,
												fcBunganAngsuran,
										...
										VALUES('$_POST[ftTrans_No]',
											   '$ftBranch_Code',
											   '$fcTotalPelunasanPokok',
											   '$fcTotalPelunasanPokok',
											   '$fcTotalPelunasanBunga',
											   '$fcTotalPelunasanAdm',
											   '$fcTotalPelunasanPblt',
											   '$fcTotalPelunasan',
											   '$_POST[fdTrans_date]',
											   '$_POST[ftCustomer_Code]',
											   'AU$thn2$bln$unik2',
...
// Update transaksi
elseif ($module=='transaksi_umum' AND $act=='update'){
 
	 mysql_query("UPDATE txpinjaman_umum_hdr SET fnStatus		= '99',
