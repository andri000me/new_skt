<?php
error_reporting(0);
include("modul/cetak_bukti_angsuran_umum/form_tgl.php");
$tgl = date("d");
$bln = date("m");
$thn = date("Y");
?> 
 ...
 var endDate;
    ...
    //    endDate = end;  
		...
		//	var startend=[startDate,endDate];
			...
			var awal=startDate;
			var akhir=endDate;
      var kel=$('#kantorbayar').val();
				
			$.ajax({
			type: 'GET',
			url: 'modul/laporan_daftar_nominatif_umum/tampil.php ',
		...
		$.ajax({
		    type: 'GET',
			url: 'excel/export_cetak_bukti_angsuran_umum.php?first='+awal+'&last='+akhir,
...
function showKantorbayar(menu) {
  sList = window.open('modul/cetak_bukti_angsuran_umum/kelompok.php', 'Print', 'width=920,height=420,scrollbars=yes');
  }
  
function showNasabah(menu) {
  var getKantorBayar = document.getElementById("kantorbayar").value;
  sList = window.open("modul/cetak_bukti_angsuran_umum/nasabah.php?kantorBayar="+ getKantorBayar +"", "Print", "width=920,height=420,scrollbars=yes");
  }
</script>
<script type="text/javascript"> 
      ...
      <header class="main-header">
        <nav class="navbar navbar-static-top">
          ...
          <li><h3 class="box-title">
              <a href="#" class="small-box-footer" onmouseover="this.style.cursor='pointer'" onClick="showKantorbayar();">
              <span id="kelom" >
              <input type="text" class="btn bg-purple dropdown-toggle" id="kantorbayar" value="Pilih Kantor Bayar"  readonly=true></a>
              </span>&nbsp;&nbsp;&nbsp;
          </h3></li> 
         
          <li><h3 class="box-title">
              <a href="#" class="small-box-footer" onmouseover="this.style.cursor='pointer'" onClick="showNasabah('e');">
              <span id="kel" >
              <input type="text" class="btn bg-purple dropdown-toggle" id="nasabah" value="Pilih Nasabah" readonly=true></a>
              </span>&nbsp;&nbsp;&nbsp;
          </h3></li> 
      ...
      <li><h3 class="box-title">
          <input type="text" class="btn bg-purple dropdown-toggle" value=<?php echo "$thn-$bln-$tgl"; ?>  name="fdTrans_date" id="fdTrans_date" value="Pilih Tanggal" readonly=true onchange="act_tgl()">
			...
			<!--		<li><h3 class="box-title">
					...
					echo "excel/export_cetak_bukti_angsuran_umum.php";?>';" >
...
if (kel=="Pilih Kantor Bayar"){
        swal({title: "Oops...",text: "Pilih Kantor Bayar Dahulu!",type:"error",closeOnConfirm: false},
          function(){swal.close();$('#kantorbayar').focus();});
            return (false);
        }else{

      $.ajax({
      type: 'GET',
      url: 'modul/cetak_bukti_angsuran_umum/tampil.php ',
      data: 'allDate='+ tgl +' sampai '+ kel + 'dan'+ nasabah, 
        dataType: 'html',
        success: function(response) {
        $('#tes').html(response);
      }
