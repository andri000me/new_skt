...
// Hapus pelunasan_umum
if ($module=='pelunasan_umum' AND $act=='delete'){
  mysql_query("DELETE FROM txpelunasan_umum_hdr WHERE fnid='$_GET[fnid]'");
  header('location:pelunasan-umum.html');
}

// Input pelunasan_umum
elseif ($module=='pelunasan_umum' AND $act=='input'){
 
	  mysql_query("INSERT INTO txpelunasan_umum_hdr(		ftTrans_No,
	  														ftBranch_Code,
															fdTrans_date,
															fcSimpanan,
															ftNotes,
	  ...
	  mysql_query("UPDATE txpinjaman_umum_nasabah_hdr set fcOutstanding = fcOutstanding - '$fcPokokAngsuran' where ftTrans_No='$_POST[ftLoan_No]' and ftCustomer_Code='$_POST[ftCustomer_Code]'");
	  
  header('location:pelunasan-umum.html'); 

}

// update pelunasan_umum
elseif ($module=='pelunasan_umum' AND $act=='update'){
 
	  mysql_query("UPDATE txpelunasan_umum_hdr SET		fnStatus		= '99',
