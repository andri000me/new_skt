          ...
          <div class="box box-primary">
			...
			<div class="table-responsive">
              <table   id="dg" 
					...
					title="Data Transaksi Pengeluaran Kasbon" 
					style="width:auto; height: auto;" 
					url="modul/transaksi_kas/aksi_pengeluaran_kas.php?module=transaksi_kas&act=getdata" 
					...
					<thead>
						<tr>
							...
							<th data-options="field:'kas_id',width:'20', halign:'center', align:'center'" hidden="true" >Dari Kas</th>
							<th data-options="field:'kas_id_txt',width:'20', halign:'center', align:'left'" >Dari Kas</th>
							<th data-options="field:'akun_id',width:'20', halign:'center', align:'center'" hidden="true">Untuk Akun</th>
							<th data-options="field:'akun_id_txt',width:'20', halign:'center', align:'center'">Untuk Akun</th>
			...
			function cetak () {
				...
				var win = window.open('modul/transaksi_kas/aksi_pengeluaran_kas.php?module=transaksi_kas&act=cetak&kode_transaksi=' + kode_transaksi + '&tgl_dari=' + tgl_dari + '&tgl_sampai=' + tgl_sampai);
				if (win) {
					win.focus();
				} else {
			...
			<script type="text/javascript"> 
				function create(){
					...
					url = "modul/transaksi_kas/aksi_pengeluaran_kas.php?module=transaksi_kas&act=create" 
				}
				function update(){
					...
					if(row){
						...
						url = 'modul/transaksi_kas/aksi_pengeluaran_kas.php?module=transaksi_kas&act=update&id=' + row.id;
						$('#jumlah ~ span input').keyup(function(){
							var val_jumlah = $(this).val();
							$('#jumlah').numberbox('setValue', number_format(val_jumlah));
						});
					...
					if (row){ 
						$.messager.confirm('Konfirmasi','Apakah Anda akan menghapus data kode transaksi : <code>' + row.id_txt + '</code> ?',function(r){  
							if (r){  
								$.ajax({
									type	: "POST",
									url		: "modul/transaksi_kas/aksi_pengeluaran_kas.php?module=transaksi_kas&act=delete&id="+row.id,
			...
			<tr style="height:35px">
				...
				<td>
					<div class="input-group date dtpicker col-md-5" style="z-index: 9999 !important;">
						<input type="text" name="tgl_transaksi" id="tgl_transaksi" style="width:150px; height:25px" required="true" readonly="readonly" />
