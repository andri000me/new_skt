	...
	<head>
		...
		function pick(a, b) {
		  if (window.opener && !window.opener.closed){
			window.opener.document.myForm.id_main.value = a;
			window.opener.document.myForm.nama_menu.value = b;
...
<table id="formsearch" class="table table-striped " onmouseover="this.style.cursor='pointer'">
	  ...
	  <thead>
		<tr>
		 <th align="center">No</th><th align="center">Nama Group Menu</th>
		 </tr>
		</thead>
		...
		<tbody>
				...
				$tampil=mysql_query("SELECT id_main,nama_menu FROM mainmenu WHERE aktif='Y' ORDER BY nama_menu");
				
				$no1 = 0;	
				while($r=mysql_fetch_array($tampil)){
				$no2=htmlentities($r['id_main']);
				$no3=htmlentities($r['nama_menu']);
				$no1++;	
