...
<html>
	<head>
		...
		function pick(a,b,c,d,f,g,h,i,j,k,l,m,n,o,p) {
...
<table id="formsearch" class="table table-striped " onmouseover="this.style.cursor='pointer'">
	  ...
	  <thead>
		...
		<tbody>
				...
				$no6=htmlentities($r['ffbunga']);
				$no7=htmlentities($r['fnJW']);
				$no8=htmlentities(number_format($r['fcOutstanding'], 0 , ',' , ',' ));
				$no9=htmlentities(number_format($r['fcPokokAngsuran'], 0 , ',' , ',' ));
				$no10=htmlentities(number_format($r['fcBunganAngsuran'], 0 , ',' , ',' ));
				$no11=htmlentities(number_format($r['fcAdmAngsuran'], 0 , ',' , ',' ));
				$no12=htmlentities(number_format($r['fcPbltAngsuran'], 0 , ',' , ',' ));
				$no13=htmlentities(number_format($r['fcTabAngsuran'], 0 , ',' , ',' ));
				$no14=htmlentities(number_format($r['fcTotalAngsuran'], 0 , ',' , ',' ));
				$no15=htmlentities(number_format($r['fcTotalAngsuran'], 0 , ',' , ',' ));
				$no16=htmlentities($r['ftNamaKelompok']);
				$no1++;	
				
				echo"
                <tr bgcolor='#d5e8e4' onmouseover=\"bgColor='#FFFF9E'\" onmouseout=\"bgColor='#d5e8e4'\" onclick=\"pick('$no2','$no3','$no4','$no5','$no6','$no7','$no8','$no9','$no10','$no11','$no12','$no13','$no14','$no15','$no16')\">
				  <td>$no1</td>
				  <td>$r[ftTrans_No]</td>   
